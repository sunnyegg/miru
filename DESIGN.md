...
Watching (rail destination) is an AniList list manager, not a poster field. The Library **Watching strip** is separate: poster tiles for CURRENT titles and catch-up, not list editing. Header carries a **Watching / Completed / Planning / Paused / Dropped / Repeating** filter row (selected = muted fill, idle = ghost — not orange). Search AniList sits in a sheet card above the list: 44px input, secondary Search, result rows with cover + title + list-status caption, and an orange **Watch** action only when the title is not already CURRENT. List cards keep the inherited row layout; **Edit** (ghost) opens a centered sheet overlay on bezel scrim where status, progress, score, dates, repeat, private, and notes can be changed — not a direct status toggle on the row. Status changes remove the row from the active filter after a successful save.
