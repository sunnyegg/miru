---
name: Miru
description: Matte MPV-bezel client — posters are the field, purple is the hit.
colors:
  background: "#111111"
  foreground: "#f2f2f2"
  accent: "#a78bfa"
...
- Single purple hit (Play, progress, selected marks)
...
A near-black bezel with phosphor type and one purple hit. Neutrals do the chrome; purple does the action.

### Primary

- **Accent Purple** (`accent`): Primary actions (search, save, start download), playback and download progress fills, the playing episode row mark, the highlighted poster outline, the current rail hairline, caret, text selection, and the scrollbar thumb. Its job is the hit, not the fill.
...
- **Phosphor** (`foreground`, `on-secondary`): Body type and type on secondary fills. `on-accent` and `on-destructive` use the field value so type on purple and on fault red stays dark.
...
**The Play Hit Rule.** Purple is the hit: primary actions, progress, the playing row mark, the highlighted poster outline, the current rail mark. Do not fill chrome, sheets, or poster tiles with it.
...
Library owns the field: main has no page padding and does not scroll as a whole. Header is 20px horizontal, 20px top, 12px bottom. Above the local poster grid, a **Watching** strip shows AniList CURRENT titles in the same square poster tiles (min 8.5rem, 12px gaps). Captions use dim progress copy (`5 / 12 · 7 left`); when a new aired episode is missing locally, the caption switches to accent **Episode N available**. Clicking a poster with local files opens the episode list; clicking one without files jumps to Search with a prefilled torrent query. Shows in the Watching strip do not repeat in the **Unlisted** strip below — local library titles not on AniList CURRENT, labeled **Unlisted**, same poster tiles. Opening an Unlisted show shows a sheet banner above the episode list: bound titles get a purple **Watch** action to add to AniList CURRENT; unbound titles get **Match AniList** first. The poster grid scrolls with the strips. Selecting a show replaces the field with a scrollable episode list; Back returns to the grid.
...
Watching (rail destination) is an AniList list manager, not a poster field. The Library **Watching strip** is separate: poster tiles for CURRENT titles and catch-up, not list editing. Header carries a **Watching / Completed / Planning / Paused / Dropped / Repeating** filter row (selected = muted fill, idle = ghost — not purple). Search AniList sits in a sheet card above the list: 44px input, secondary Search, result rows with cover + title + list-status caption, and a purple **Watch** action only when the title is not already CURRENT. List cards keep the inherited row layout; **Edit** (ghost) opens a centered sheet overlay on bezel scrim where status, progress, score, dates, repeat, private, and notes can be changed — not a direct status toggle on the row. Status changes remove the row from the active filter after a successful save.
...
Focus is a 2px `ring` outline, offset 2px, on the same purple as primary actions. Text selection is purple on field-dark. The caret is purple. Scrollbars are thin: purple thumb on bezel track. Color changes use 200ms. `prefers-reduced-motion: reduce` collapses transition duration.
...
- **Play / primary:** Purple fill, field-dark type, 0 20px padding, medium 0.875rem. Search, Downloads start, and Settings save use this fill; min-width 96px only where the design locks it.
...
- **Hover / Focus:** Play has no fill hover; secondary and muted swap panel steps. Focus is the global 2px purple outline, offset 2px.
...
- **Focus:** 2px purple outline, offset 2px (`ring`).
...
- **Current:** Muted fill, phosphor type, left hairline in purple (`border-l` + `accent`).
...
Square tiles, min 8.5rem, 12px gaps. Cover is `object-cover` on a muted square; missing cover is muted with dim title anchored at the bottom. Caption is medium body + label: AniList watch progress for linked shows (`5 / 12 · 7 left`), or local file count when not linked. Highlighted tile (currently playing show): 2px purple outline, offset 2px. No Play on the tile.
...
- **Do** mark the current rail destination with a left purple hairline on a muted fill.
- **Do** paint playback and download progress as purple fills on muted tracks.
...
- **Don't** fill chrome, nav, or poster tiles with purple.
