...
export function DownloadJobCard({
  ...
  let actions = (
    <div className="flex flex-wrap gap-2">
      <Button type="button" variant="destructive" onClick={() => onPendingDelete(item.id)} disabled={busy}>
        Delete
      </Button>
    </div>
  )
  ...
  if (isQueued) {
    actions = (
      <div className="flex flex-wrap gap-2">
        <Button type="button" variant="destructive" onClick={() => onCancel(item.id)} disabled={busy}>
          Cancel
        </Button>
      </div>
    )
    ...
    actions = (
      <div className="flex flex-wrap gap-2">
        <Button type="button" variant="secondary" onClick={() => onPause(item.id)} disabled={busy}>
          Pause
        </Button>
        {isSeeding && (
          <Button type="button" onClick={() => onFinish(item.id)} disabled={busy}>
            Stop seeding
          </Button>
        )}
        <Button type="button" variant="destructive" onClick={() => onCancel(item.id)} disabled={busy}>
    ...
    actions = (
      <div className="flex flex-wrap gap-2">
        <Button type="button" onClick={() => onResumeSeeding(item.id)} disabled={busy}>
          Resume
        </Button>
        <Button type="button" variant="secondary" onClick={() => onFinish(item.id)} disabled={busy}>
          Stop seeding
        </Button>
      </div>
    )
    ...
    actions = (
      <div className="flex flex-wrap gap-2">
        <Button type="button" variant="secondary" onClick={() => onRemove(item.id, false)} disabled={busy}>
          Remove from list
        </Button>
        <Button type="button" variant="destructive" onClick={() => onConfirmDeleteFiles(item.id)} disabled={busy}>
          Remove and delete files
        </Button>
        <Button type="button" variant="ghost" onClick={onClearPendingDelete} disabled={busy}>
          Back
        </Button>
      </div>
    )
  ...
  const statusLine = group === 'completed'
